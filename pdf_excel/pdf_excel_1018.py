...
path = r'C:\Users\15644\Desktop\pdf_file\test_pdf_list\test_10.pdf'
...
def parse():
    ...
    for page in PDFPage.create_pages(doc): # doc.get_pages() 获取page列表
        ...
        if len(page_container[rows_num - 1]) != len(page_container[rows_num - 2]):
            page_container[rows_num - 1],unused_flag = align_row(page_container[rows_num - 2],page_container[rows_num - 1])
        ...
        if len(page_container[0]) != len(page_container[1]) or len(page_container[1]) != len(page_container[2]) or len(page_container[2]) != len(page_container[3]) or len(page_container[3]) != len(page_container[4]):
            ...
            for i in range(len(rows_length)):
                if rows_length[i] < max_length:
                    page_container[i],flag_for_title = align_row(the_max_row,page_container[i])
        ...
        for i in range(len(page_container)):
            for j in range(len(page_container[i])):
                ...
                if page_container[i][j] == None:
                    ...
                    ws.merge_cells(start_row=i + 1 + text_number, start_column=1, end_row=i + 1 + text_number, end_column=len(page_container[i]))
                    break 
                else:
...
# 合并表头，并未对位置信息进行同步
def compile_row(row_list):
    res_row = []
    for j in range(len(row_list[0])):
        temp_array = ''
        for i in range(len(row_list)):
            if row_list[i][j] != None:
                temp_array += row_list[i][j]['value']
        res_row.append({'value':temp_array,'location':[]})
    return res_row
...
def align_row(row_list_max,row_list_min):
    ...
    row_list_min = []
    if len(loc_for_min) == 1 and len(adjust_for_min) > 1:
        row_list_min.append(temp_array[0])
        for i in range(len(row_list_max) - 1):
            row_list_min.append({'value':'','location':[]})
    else:
        flag_title = False
    ...
    return row_list_min,flag_title
